...
RESULTS_DIR = os.path.join('../..','equilibration/2/equil_results')
MOLECULES_DONE_FILEPATH = os.path.join(RESULTS_DIR, 'equil_done.json') #file which listed completed runs
#Folder where the equilibrated PDBs will be saved
OUTPUT_DIR = os.path.join('../..','equilibration/2/equil_pdbs/')
...
def run_equil(job_id, n_jobs):
    # get every ID/PDB of all the different mixtures
    #make sure to change this for the trial set that you're working on
    pdb_dir = os.path.join('../..', 'equilibration/2/output_files/')
    ...
    for i, molecule_id in enumerate(molecule_ids):
        ...
        #Get the PDB filepath
        PDB_filepath = os.path.join(pdb_dir, molecule_id +'.pdb')
        #Grab the XML of the corresponding PDB file
        #XML_filepath = os.path.join(pdb_dir.replace("output_files", "GS_systems"), molecule_id[:-6] +'.xml')
        XML_filepath = os.path.join(pdb_dir.replace("output_files", "GS_systems"), molecule_id +'_GAFF_tip3p.xml')
        print("XML_filepath:",XML_filepath)
        #Creat path/name for the equilibrated PDB
        PDBfilepath = os.path.join(OUTPUT_DIR, molecule_id +'_GAFF_tip3p_equil.pdb')
