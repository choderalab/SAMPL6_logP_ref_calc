...
for d in ../t3/dry_yank_output/* ; do
    ...
    # Convert nc to dcd
    yank analyze extract-trajectory --trajectory="$path_to_output_system_directory/anti_trial3_solvent1.dcd" --netcdf="$d/solvent1.nc" --state=0
    yank analyze extract-trajectory --trajectory="$path_to_output_system_directory/anti_trial3_solvent2.dcd" --netcdf="$d/solvent2.nc" --state=0
done
