...
RESULTS_DIR = os.path.join('.','wet_yank_results') #where the json file will be stored
MOLECULES_DONE_FILEPATH = os.path.join(RESULTS_DIR, 'wet_yank_mols_done.json') #this json file keeps track of molecule that are done

OUTPUT_DIR = os.path.join('.','wet_yank_outputs')# where the yank results/output will be stored
...
def run_yank(job_id, n_jobs):
    ...
    openmm_system_dir = "./t1_wet_oct_XMLs"
    
    # Path to YANK template script (YAML file)
    yank_script_template_filepath = 'wet_yank.yaml' #don't need to change this
    ...
    molecules_files_pattern = os.path.join(pdb_dir, '*_water_octanol_0.0_0.2705_0.7295_SMIRNOFF_tip3pFB_equil.pdb')
    molecule_ids = [os.path.basename(molecule_file)[:-4] for molecule_file in glob.glob(molecules_files_pattern)] # makes a list of just the system IDs (ie SM13_octanol_0.0_1.0_SMIRNOFF_tip3pFB_equil instead of SM13_octanol_0.0_1.0_SMIRNOFF_tip3pFB_equil.pdb)
    ...
    for i, molecule_id in enumerate(molecule_ids):
        ...
        phase2_molecule_id = phase1_molecule_id.replace("_water_octanol_0.0_0.2705_0.7295_SMIRNOFF_tip3pFB_equil", "_water_0.0_1.0_SMIRNOFF_tip3pFB_equil")
...
if __name__ == '__main__':
    ...
    print("Started running wet_run_yank.py ...")
    ...
    print("Done running wet_run_yank.py!")
