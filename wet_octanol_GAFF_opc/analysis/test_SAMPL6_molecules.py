...
trials = glob.glob("/data/chodera/misik/SAMPL6_logP_ref_calc/wet_octanol_GAFF_opc/t*/wet_yank_output/")
...
for idx in range(len(trials)):
    ...
    for root, dirs, files in os.walk(str(trials[idx]), topdown=False):
        for name in dirs:
            if name[0:2] == "SM" and "_GAFF_opc_equil" in name:
    ...
    # Added to see the results of partial analysis before errror
        trial_num = trials[idx].split("/")[-3]
        filename = "SAMPL6_mols_trial_" + trial_num + "_GAFF_opc_wetoct.json"
